...
Plugin URI: https://airtonvancin.com/plugin/video-destacado
...
function video_destaque_metabox()
{
  ...
  <img style="<?php echo empty($id_video) ? 'display: none;' : 'display: block;'; ?>" class="thumb"
    src="<?php echo esc_url('https://img.youtube.com/vi/' . $id_video . '/0.jpg'); ?>"
    alt="<?php echo esc_attr(get_the_title($post->ID)); ?>" />

  <ul id='video-destaque'>
    <li><span><?php esc_html_e('ID do Vídeo:', 'video-destacado'); ?></span> <input type="text" id="id_video"
        name="id_video"
        value="<?php echo esc_attr($id_video); ?>" /><small><?php esc_html_e('Ex: www.youtube.com/watch?v=', 'video-destacado'); ?><b>XdMD4LrC4wY</b></small>
    </li>
    <li>
    ...
    <li>
      <input type="submit" name="add" class="button add"
        value="<?php echo esc_attr__('Adicionar', 'video-destacado'); ?>" />
      <input type="submit" name="del" class="button del"
        value="<?php echo esc_attr__('Remover', 'video-destacado'); ?>" />
    </li>
  </ul>
  <?php
}
